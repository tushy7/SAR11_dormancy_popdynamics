...
function objective(optParams, phi, dt) 
    ...
    timePerCycle = round(Int, dt / 0.1) + 1 #determines that there are 6 data points per cycle
    endingCycles = 3 * timePerCycle # total 18 cycles 
...
function optimizeDormancy(phi, dt) 
    ...
    prob = OptimizationProblem(optf, [0.66, 0.1]; lb=[0.1, 0.01], ub=[2.0, 5.0]) #like writing down the intial question with initial guesses etc. 

    sol = solve(prob, LBFGS(); maxiters=100, maxtime=500.0) #can start max iters lower for now if it's slow 

    return sol.u[2]  #return optimal dormancy rate q AND average active population
end
...
function heatMap(phiRange::Tuple{Float64, Float64}, dtRange::Tuple{Float64, Float64}; outputFile::String="heatMapData.csv")
    phis = range(phiRange[1], phiRange[2], length=4) #section virus count range into 8 subsets
