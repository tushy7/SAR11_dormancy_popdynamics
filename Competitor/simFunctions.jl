...
using OrdinaryDiffEq, DiffEqCallbacks
    ...
    beta_main   = defaultBetaMain
    beta_menace = defaultBetaMenace

    dormancyRate = withDormancy ? optParameters[2] : 0.0
    initialState = [initialCellCount, initialCellCount, nutrientReset,
                    initialVirusLoadMain, 0.0,
                    includeCompetitors ? initialCompetitorCount : 0.0,
                    initialVirusLoadMenace]
    ...
    params = ([optParameters[1], dormancyRate], phi, beta_main, phi_menace, beta_menace, mp, withDormancy, isSporeFormer)
    ...
    quiDil     = copy(sol[2,:])
    nutDil     = copy(sol[3,:])
    virMainDil = copy(sol[4,:])
    infDil     = copy(sol[5,:])
    menDil     = copy(sol[6,:])
    virMenDil  = copy(sol[7,:])
    time       = copy(sol.t)
    ...
    for i in 1:(num_full - 1)
        ...
        total_main = dilState[1] + dilState[2] + dilState[5]  # A + Q + infectedQ

        if total_main < extinctionThreshold
            dilState[1] = 0.0   # Active
            dilState[2] = 0.0   # Quiescent
            dilState[5] = 0.0   # InfectedQ
        end
        if i > 2 && dilState[6] < extinctionThreshold
            dilState[6] = 0.0
        end
        ...
        append!(virMainDil, newSol[4,:])
        append!(infDil, newSol[5,:])
        append!(menDil, newSol[6,:])
        append!(virMenDil, newSol[7,:])
        append!(time, newSol.t .+ time[end])

        checkExtinct!(actDil, extinctionThreshold)
        checkExtinct!(quiDil, extinctionThreshold)
        checkExtinct!(infDil, extinctionThreshold)
        checkExtinct!(menDil, extinctionThreshold)
    end
    ...
    if run_partial_last && rem_time > 1e-9
        ...
        total_main = dilState[1] + dilState[2] + dilState[5]
        ...
        if dilState[6] < extinctionThreshold
            dilState[6] = 0.0
        end
    ...
    return (actDil, quiDil, nutDil, virMainDil, infDil, menDil, virMenDil, time)
end
