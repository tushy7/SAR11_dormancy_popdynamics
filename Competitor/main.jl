...
results = heatMap(phiRange, dtRange;
                  nphi=8, ndt=6, growth=umax,
                  dilute_dormant=false,
                  dilution_effect=dilEffect)

using CSV
CSV.write("heatmap_results_$(dilEffect).csv", results)




df, summary = heatMapTracked((1e-10, 1e-6), (0.5, 21.0);
                             nphi=8, ndt=8, growth=umax, dilute_dormant = false, dilution_effect=dilEffect)
